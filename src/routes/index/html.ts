...
function generateFirstInLastInCrossTab(
  features: CompatResult[]
): BrowserCrossTabResult {
  ...
  for (const feature of features) {
    ...
    if (
      feature.stableStats.last.browser in
        output[feature.stableStats.first.browser] ==
      false
    ) {
      output[feature.stableStats.first.browser][
...
function generateAverage(features: CompatResult[]) {
  ...
  const categories: {
    [Z in ValidFeatures]: { total: number; featureCount: number };
  } = initApiCounts();
  const firstLanding: { [K: number]: typeof categories } = {
    // << Fffark the typeof worked as I guessed. Very cool Typescript. Very cool.
  };
  const firstBrowser: {
    [K in number]: { [B in BrowserName]?: typeof categories };
  } = {};
  const lastBrowser: {
    [K in number]: { [B in BrowserName]?: typeof categories };
  } = {};
  ...
  for (const feature of features) {
    ...
    if (feature.category !== undefined) {
      ...
      firstLanding[year][feature.category].total +=
        feature.stableStats.ageInDays;
      firstLanding[year][feature.category].featureCount++;

      firstBrowser[year][first.browser][feature.category].total +=
        feature.stableStats.ageInDays;
      firstBrowser[year][first.browser][feature.category].featureCount++;
      lastBrowser[year][last.browser][feature.category].total +=
        feature.stableStats.ageInDays;
...
function renderResults({
  ...
  <table class=tabular>
    ...
    <thead>
      <tr>
        <th></th>
        ${[...selectedBrowsers].map(
          (key) => template`<th>Last in ${helper.getBrowserName(key)}</th>`
        )} 
      </tr>
    </thead>
    <tbody>
      ${[...selectedBrowsers].map(
        (firstInKey) => template`<tr>
        <th scope="row">First in ${helper.getBrowserName(firstInKey)}</th>
        ${[...selectedBrowsers].map(
          (lastInKey) =>
            template`<td>${tablulateSummary[firstInKey][lastInKey]}</td>`
        )}
        </tr>`
      )} 
    </tbody>
  ...
  <p>${averages.featureCount} APIs took an average of ${(
    averages.total / averages.featureCount
  ).toFixed(2)} days to become available to use.</p>
  <p>API breakdown:</p>
  <ul>
  ${[...selectedFeatures].map(
    (category) =>
      `<li>${featureConfig[category].name}: ${(
        averages.categories[category].total /
        averages.categories[category].featureCount
      ).toFixed(2)} days</li>`
  )}
  </ul>
  ...
  <table>
    ...
    <thead>
      <tr>  
        ...
        ${[...selectedFeatures].map(
          (category) =>
            `<th>${featureConfig[category].name} APIs</th><th>${featureConfig[category].name} TTA</th>`
        )}
      </tr>
      </thead>
    <tbody>
${template`${Object.entries(averages.firstLanding).map(([year, categories]) => {
  return template`<tr>
    ...
    ${[...selectedFeatures].map(
      (category) =>
        `<td>${categories[category].featureCount}</td><td>${(
          categories[category].total / categories[category].featureCount
        ).toFixed(2)}</td>`
    )}
    </tr>`;
})}`}
  ...
  <table>
    ...
    <thead>
      <tr>  
        ...
        ${[...selectedFeatures].map(
          (category) =>
            `<th>${featureConfig[category].name}</th><th>${featureConfig[category].name} TTA</th>`
        )}
      </tr>
      </thead>
    <tbody>
${template`${
  Object.entries(averages.firstBrowser).map(
    ([year, browsers]) =>
      template`
    <tr>
      <th colspan="${
        selectedFeatures.size * 2 + 1
      }" scope="colgroup">${year}</th>
    </tr>
    ${template`${Object.entries(browsers).map(
      ([browser, categories]) =>
        `<tr>
  ...
  <h4>Plodders</h4>
  ${
    selectedBrowsers.size == 2
      ? `<aside>When there are only 2 browsers this table is the inverse of the "Sprinters".</aside>`
      : ``
  }
  ...
  <table>
    ...
    <thead>
      <tr>  
        ...
        ${[...selectedFeatures].map(
          (category) =>
            `<th>${featureConfig[category].name} count</th><th>${featureConfig[category].name} TTA</th>`
        )}
      </tr>
      </thead>
    <tbody>
${template`${
  Object.entries(averages.lastBrowser).map(
    ([year, browsers]) =>
      template`
    <tr>
      <th colspan="${
  ...
  <h3>Raw Data</h3>
  Quick Links: <ul>${[...selectedFeatures].map(
    (selectedFeature) =>
      template`<li><a href="#${selectedFeature}-table">${featureConfig[selectedFeature].name}</a></li>`
  )}</ul>
  ${features.map((feature) => {
    ...
    <td>${
      ...
      "spec_url" in feature && feature.spec_url != undefined
        ...
        : template``
    }</td><td>${helper.getBrowserName(
      feature.stableStats.first.browser
    )}</td><td>${feature.stableStats.first.added.toLocaleDateString()}</td>
    <td>${helper.getBrowserName(
      feature.stableStats.last.browser
