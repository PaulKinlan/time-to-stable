...
function renderResults({ helper, browserList, stableFeatures, selectedBrowsers, selectedFeatures, featureConfig }: { bcd: CompatData; browsers: Browsers; helper: BrowsersHelper; browserList; stableFeatures: CompatResult[]; selectedBrowsers: Set<BrowserName>; selectedFeatures: Set<ValidFeatures>; featureConfig: FeatureConfig; }): ReadableStream<any> {
  ...
  <h4>Average time for features to become available across ${browserList}</h4>
  <p>Selected features: ${(averages.total / averages.featureCount).toFixed(2)} days</p>
  ...
  <table>
    <caption>If a feature landed in the earliest browser in 20XX it took Y days on average to become available in the last browser (when considering ${browserList}).</caption>
    <thead>
      <tr>  
        <th></th>
        ${[...selectedFeatures].map(category => `<th>${featureConfig[category].name}</th>`)}
      </tr>
      </thead>
    <tbody>
${template`${Object.entries(averages.firstLanding).map(([year, categories]) => {
    return template`<tr>
    <th>${year}</th>
    ${[...selectedFeatures].map(category => `<td>${(categories[category].total / categories[category].featureCount).toFixed(2)}</td>`)}
    </tr>`
  ...
  <p>There is a natural tension on the web. There are browsers that push on the platform adding capabilities ahead of others, and there are browsers that are slower at adding features.</p>
  <p>If you think of the web as a convoy, the convoy only moves at the pace of the slowest member.</p>
  <p>This section is designed to show which browsers are pushing and which are pulling.</p>
  <h4>Sprinters</h4>
  ...
  <table>
    <caption>For a given year, if a feature landed in Browser X first, how long did it take to be available in ${browserList}</caption>
    <thead>
      <tr>  
        <th>Year</th>
        ${[...selectedFeatures].map(category => `<th>${featureConfig[category].name}</th>`)}
...
${template`${
  ...
  template`
    <tr>
      <th colspan="${selectedFeatures.size + 1}" scope="colgroup">${year}</th>
    </tr>
    ${template`${Object.entries(browsers).map(([browser, categories]) => 
      `<tr>
        <th>&nbsp;&nbsp;${helper.getBrowserName(browser)}</th>
        ${[...selectedFeatures].map(category => 
          `<td>${(categories [category].total / categories[category].featureCount).toFixed(2)}</td>`).join("")}
      </tr>`
    )}`}
  `)// 
}`}
  ...
  <table>
    <caption>For a feature that first landed in year X, how long did it take for the last browser to catch up across ${browserList}.</caption>
    <thead>
      <tr>  
        <th>Year</th>
        ${[...selectedFeatures].map(category => `<th>${featureConfig[category].name}</th>`)}
