  ...
  for (const feature of features) {
    ...
    for (const browser of featureIn) {
      ...
      for (const browserNotIn of featureNotIn) {
        if (browserNotIn in output[browser] == false)
          output[browser][browserNotIn] = 0;
...
function renderResults({
    ...
    <thead>
      <tr>
        ...
        ${[...selectedBrowsers].map(
          (key) => template`<th>Not in ${helper.getBrowserName(key)}</th>`
        )} 
      </tr>
    </thead>
    <tbody>
  ...
  ${features.map((feature) => {
    ...
    )}</td><td>${feature.stableStats.first.added.toLocaleDateString()}</td><td>${(
      (Date.now() - feature.stableStats.first.added.getTime()) /
      (1000 * 24 * 60 * 60)
    ).toFixed(0)}</td><td>${renderFeatureNotes(feature)}</td></tr>`;
