...
function renderResults({
    ...
    <thead>
      <tr>
        ...
        ${[...selectedBrowsers].map(
          (key) =>
            template`<th>Last removed from ${helper.getBrowserName(key)}</th>`
        )} 
      </tr>
    </thead>
    <tbody>
      ${[...selectedBrowsers].map(
        ...
        <th scope="row">First removed from ${helper.getBrowserName(
          firstInKey
        )}</th>
        ${[...selectedBrowsers].map(
          (lastInKey) =>
  ...
  ).toFixed(2)} days to be removed</p>
  <p>API breakdown:</p>
  <ul>
...
${template`${Object.entries(averages.firstLanding).map(([year, categories]) => {
  ...
  ${features.map((feature) => {
    ...
    <td>${
      ...
      feature.removedStats.first.browser
    )}</td><td>${feature.removedStats.first.added.toLocaleDateString()}</td>
    <td>${helper.getBrowserName(
      feature.removedStats.last.browser
    )}</td><td>${feature.removedStats.last.added.toLocaleDateString()}</td><td>${
      feature.ageInDays
    }</td></tr>`;
