...
export default function render({
  ...
  <link rel="alternate" type=”application/rss+xml” href="/when-stable?${renderBrowsersQuery(
    browsers,
    selectedBrowsers
  )}&${renderFeaturesQuery(featureConfig, selectedFeatures)}&type=rss">
  <style>
   ...
   ${
     warnings.length == 0
       ? features.map((feature) => {
           ...
           let heading;
           const date =
             feature.stableStats.last.added.getFullYear() +
             "/" +
